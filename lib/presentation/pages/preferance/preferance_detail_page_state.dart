...
class PreferanceDetailPageState extends State<PreferanceDetailPage> {
  ...
  Future<void> _generateNewTask() async {
    ...
    try {
      ...
      if (suggestion.isNotEmpty) {
        ...
        basePrompt += "Based on your idea ('$suggestion'), find an activity related to '$preferenceName' and apply that concept. ";
      } else {
        // Default task if no suggestion is provided
        basePrompt += "Find a short article or chapter about '$preferenceName' and summarize the main points in three sentences. ";
      }
      
      final task = basePrompt + "(Difficulty based on your ${percentage}% progress.)";
  ...
  void _acceptGeneratedTask() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Task accepted! Progress increased to ${updatedPreferance.percentage}%.")),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bloc = context.read<PreferanceBloc>();
    final preferance = widget.preferance; // Use widget.preferance in a stateful widget

    return Scaffold(
      appBar: AppBar(
        title: Text(preferance.preferance),
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                label: Text(_generatedTask == null ? "Generate New Task" : "Regenerate Task"),
